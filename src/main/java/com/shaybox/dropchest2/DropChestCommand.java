...
public class DropChestCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		DropChestPlayer dcPlayer = DropChestPlayer.getPlayer(player.getUniqueId());

		if (!"dropchest".equals(cmd.getName().toLowerCase())) return true;

		if (!player.hasPermission("dropchest.default")) {
		...
		if (args.length == 0) {
			...
			String range = player.hasPermission("dropchest.radius.unlimited") ? "unlimited" : maxRange;
			...
			if (player.hasPermission("dropchest.radius.set"))
				player.sendMessage(YELLOW + "/dropchest radius <radius>");
			if (player.hasPermission("dropchest.owner.set"))
			...
			if (player.hasPermission("dropchest.filter"))
				player.sendMessage("/dropchest filter {suck|push|pull} [{chest} {itemid|itemtype|clear|all}]");
			return true;
		}
		...
		switch (args[0].toLowerCase()) {
			case "add": {
				...
				if (args.length == 2 && player.hasPermission("dropchest.radius.set")) {
					radius = Integer.parseInt(args[1]);
					int maxRadius = plugin.getConfig().getInt("max-radius");
					if (radius > maxRadius && !player.hasPermission("dropchest.radius.unlimited"))
						radius = maxRadius;
				}
				dcPlayer.setRadius(radius);
				dcPlayer.setAction("ADD");
			...
			case "radius": {
				...
				if (args.length < 2) {
					...
					player.sendMessage(GOLD + "/dropchest radius <radius>");
					return true;
				}

				int radius = Integer.parseInt(args[1]);
				int maxRadius = plugin.getConfig().getInt("max-radius");
				if (radius > maxRadius && !player.hasPermission("dropchest.radius.unlimited")) radius = maxRadius;
				dcPlayer.setRadius(radius);
				dcPlayer.setAction("RADIUS");
			...
			case "owner": {
				...
				if (args.length < 2) {
					...
					player.sendMessage(GOLD + "/dropchest owner <player>");
					return true;
				}
				...
				dcPlayer.setOwner(newOwner);
				dcPlayer.setAction("OWNER");
